...
public class Collaborative_filter {
	...
	public BookVector[] recommendBook(int userID, int amount){
		PriorityQueue<KNNUserVector> users = findSimiUser(userID,10);
		/*
	...
	private double CosineSimilarity(UserVector curUser, UserVector target){
		...
		for(String ISBN: allBook){
			/*
			Byte score1 = curRatingRecord.get(ISBN);
			Byte score2 = targetRatingRecord.get(ISBN);
			if(score1 == null)
				num1 = 0.0;
			else
				num1 = score1;
		...
		//System.out.println("simi:" + simi + "   m1:" + m1 + "   m2:" + m2);
		if(m1 == 0.0 || m2 == 0.0)
			return 0.0;
		return simi / ((Math.sqrt(m1)) * (Math.sqrt(m2)));
	}
	private PriorityQueue<KNNUserVector> findSimiUser(int userID, int amount){
		PriorityQueue<KNNUserVector> simiQueue = new PriorityQueue<KNNUserVector>(amount, new KNNCompare());
	...
	private BookVector [] knnSearch(PriorityQueue<KNNUserVector> pq,int amount){
		...
		HashSet<String> allBook = new HashSet<String>();
		PriorityQueue<KNNBookVector> bookPQ = new PriorityQueue<KNNBookVector>(amount, new KNNBookCompare());
		int i = 0;
		...
		for(String ISBN: allBook){
			...
			avg/=users.length;
			//System.out.println("ISBN:" + ISBN + "    avg: " + avg);
